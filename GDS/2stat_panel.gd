...
func do_Battery(raw_data : PackedByteArray) -> Dictionary:
	var val : float = raw_data.slice(0,1).decode_u8(0) / 10.0
	var val2 : float = raw_data.slice(1,2).decode_u8(0) / 10.0
...
func do_Position(raw_data : PackedByteArray) -> Dictionary:
	...
	var Long : float = stream.get_float()
	stream.seek(4)
	var Lat : float = stream.get_float()
	stream.seek(8)
	var Height : int = stream.get_u16()
	
	#Logger.add_Log(str(Long) + " " + str(Lat) + " " + str(Height))
...
func update_on_Raw(message : PackedByteArray):
	...
	if container["MessageType"] == 0:
		$DragComponent2/Imager.parse_Batch(message.slice(21))
	else:
		$DragComponent2/Sounder.parse_Batch(message.slice(21))
...
func _on_option_panel_port_change(index : int, port: String) -> void:
	$DragComponent2/Node/Container/ArduinoReader.port_change(index, port)
...
func _on_arduino_reader_on_error(really: bool) -> void:
	if really:
		...
		error_weight = 0
	if error_weight > 2:
		$DragComponent2/Node/StabilityState.texture = ImageTexture.create_from_image(Image.load_from_file("Resources/Textures/NEIN.png"))
	elif error_weight > 0:
		$DragComponent2/Node/StabilityState.texture = ImageTexture.create_from_image(Image.load_from_file("Resources/Textures/MID.png"))
	else:
		$DragComponent2/Node/StabilityState.texture = ImageTexture.create_from_image(Image.load_from_file("Resources/Textures/OK.png"))
