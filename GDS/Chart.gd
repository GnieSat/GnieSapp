...
func _ready():
	...
	var x: PackedFloat32Array = ArrayOperations.multiply_float(range(0, 100), 1)
	...
	cp.title = "DataChart"
	cp.x_label = "Time"
	cp.y_label = "Data obtain attempt"
	cp.x_scale = 5
	cp.y_scale = 1
	...
	f1 = Function.new(
		x, y, "Acquisition", # This will create a function with x and y values taken by the Arrays 
