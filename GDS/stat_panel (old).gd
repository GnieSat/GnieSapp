...
func do_Light(raw_data : String) -> Dictionary:
	var result : Dictionary = {
		"R" : str(Math.bits_to_uint16(raw_data.substr(0,16))),
		"G" : str(Math.bits_to_uint16(raw_data.substr(16,16))),
		"B" : str(Math.bits_to_uint16(raw_data.substr(32,16))),
		"Clear" : str(Math.bits_to_uint16(raw_data.substr(48,16))),
		"IR" : str(Math.bits_to_uint16(raw_data.substr(64,16))),
	}
	return result

func do_Position(raw_data : String) -> Dictionary:
	var result : Dictionary = {
		"Long" : str(Math.bits_to_float32(raw_data.substr(0,32))),
		"Lat" : str(Math.bits_to_float32(raw_data.substr(32,32))),
		"Height" : str(Math.bits_to_uint16(raw_data.substr(64,16))),
		"Speed" : str(Math.bits_to_uint8(raw_data.substr(80,8)))
	}
	return result
...
func _on_arduino_reader_on_update(message: PackedByteArray) -> void:
	#if messagep[0] == "G":
		#
	#else:
